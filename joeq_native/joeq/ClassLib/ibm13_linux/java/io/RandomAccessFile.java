...
public abstract class RandomAccessFile {
    ...
    private static int readBytes(java.io.RandomAccessFile dis, byte b[], int off, int len, java.io.FileDescriptor fd) throws java.io.IOException {
    ...
    private static void writeBytes(java.io.RandomAccessFile dis, byte b[], int off, int len, java.io.FileDescriptor fd) throws java.io.IOException {
        int fdnum = Reflection.getfield_I(fd, FileDescriptor._fd);
        // check for index out of bounds/null pointer
        if (len < 0) throw new IndexOutOfBoundsException();
        byte b2 = b[off+len-1];
