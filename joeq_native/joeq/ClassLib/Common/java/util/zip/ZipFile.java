...
public abstract class ZipFile implements ZipConstants {
    ...
    */
    ...
    public int size() {
        Hashtable entries = this.entries;
        if (TRACE) System.out.println(this+": getting size = "+entries.size());
        return entries.size();
    }
    public void close() throws java.io.IOException {
        if (TRACE) System.out.println(this+": closing file");
    ...
    throws java.io.IOException {
        ...
        if (TRACE) System.out.println(this+": getting input stream for "+ze+" = "+in);
        switch (ze.getMethod()) {
        ...
        case java.util.zip.ZipEntry.DEFLATED:
            ...
            if (TRACE) System.out.println(this+": using inflater "+inflater);
    ...
    private static class ZipFileInputStream extends java.io.InputStream {
        ...
        public int read(byte b[], int off, int len) throws java.io.IOException {
            ...
            if (TRACE) System.out.println(this+": reading off="+off+" len="+len);
            if (count == 0) {
                return -1;
            }
            if (len > count) {
        ...
        public int read() throws java.io.IOException {
            ...
            ZipFile zf = this.zf;
            long pos = this.pos;
            if (TRACE) System.out.println(this+": reading pos="+pos);
            int n = zf.read(pos);
            if (n == -1) {
                throw new java.util.zip.ZipException("premature EOF");
            }
            this.pos = pos+1;
            this.count = count-1;
            if (TRACE) System.out.println(this+": new pos="+(pos+1));
            if (TRACE) System.out.println(this+": new count="+(count-1));
            return n;
        }
        ...
        public long skip(long n) {
            ...
            if (TRACE) System.out.println(this+": skipping "+n);
            long pos = this.pos;
            this.pos = pos+n;
            this.count = count-n;
            if (TRACE) System.out.println(this+": new pos="+(pos+n));
            if (TRACE) System.out.println(this+": new count="+(count-n));
            return n;
        }
        ...
        private void readLOC() throws java.io.IOException {
            ...
            if (TRACE) System.out.println(this+": reading LOC, offset="+offset);
            ...
            this.count = count;
            if (TRACE) System.out.println(this+": count="+count);
            long pos = ze.getOffset() + LOCHDR + ZipFile.get16(locbuf, LOCNAM) + ZipFile.get16(locbuf, LOCEXT);
            this.pos = pos;
            if (TRACE) System.out.println(this+": pos="+pos);
            long cenpos = zf.cenpos;
            if (TRACE) System.out.println(this+": cenpos="+cenpos);
            if (pos + count > cenpos) {
                throw new java.util.zip.ZipException("invalid LOC header format");
            }
        }
    }
    private /*synchronized*/ int read(long pos, byte b[], int off, int len)
        throws java.io.IOException
    {
        ...
        if (TRACE) System.out.println(this+": number read="+n);
        if (TRACE) System.out.println(this+": current pos="+(pos+n));
        if (n > 0) {
            this.pos = pos+n;
        }
        return n;
    ...
    private /*synchronized*/ int read(long pos) throws java.io.IOException {
        ...
        int n = raf.read();
        if (TRACE) System.out.println(this+": byte read="+n);
        if (n > 0) {
            this.pos = pos+1;
        }
        return n;
    ...
    private void readCEN() throws java.io.IOException {
        if (TRACE) System.out.println(this+": reading CEN...");
        // Find and seek to beginning of END header
        long endpos = this.findEND();
        if (TRACE) System.out.println(this+": endpos="+endpos);
        ...
        for (int off = 0; off < cenlen; ) {
            ...
            off += entrysize;
            if (TRACE) System.out.println(this+": entrysize="+entrysize+" offset="+off);
        ...
        for (pos = len; pos > markpos; ) {
            ...
            while (--n > 0) {
                if (get32(buf, n) == ENDSIG) {
                    ...
                    if (TRACE) System.out.println(this+": findEND comlen="+comlen);
                    ...
                    raf.seek(endpos);
                    this.pos = endpos;
                    if (TRACE) System.out.println(this+": findEND pos=endpos="+endpos);
                    return endpos;
                }
            }
        }
